...
namespace BiomeLords.Patches
{
    ...
    ///
    /// Every prefix runs at Priority.First. ComfyQuickSlots prefixes four of these
    /// same methods and returns false to answer them itself — and in Harmony 2 a
    /// prefix returning false skips the prefixes that come AFTER it, so at equal
    /// priority (CQS loads first) our height correction would simply never run.
    /// Going first also means CQS computes its own answer from the corrected height.
    ///
    /// Correct capacity is only half the job: the extra rows must also be reachable
    /// by the slot-finder. See <see cref="FeatherweightEmptySlotPatches"/>.
    /// </summary>
    public static class FeatherweightCapacityPatches
    {
        ...
        // Valheim 1.0 appended `bool pickedUp` and `bool dropIfFullInv` to this overload.
        [HarmonyPatch(typeof(Inventory), nameof(Inventory.AddItem),
            new System.Type[] {
